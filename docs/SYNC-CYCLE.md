...
The **client settings** check (`AddClientSettingsResultAsync` → `IManagedClient.GetConflictingSettingsAsync`) reports the client's own options that undo the synchronized port: a randomised listening port, and the client's built-in UPnP/NAT-PMP mapping. All four clients already write these to a safe value on every `SetListeningPortAsync`, so this check exists for the window in between - a user can re-enable one at any time and nothing corrects it until the VPN's port next changes, which may be days. It is Diagnostics-only rather than per-cycle: the correction happens at the next port write regardless, so paying an extra client round-trip every cycle would buy only earlier notice. The contract returns an empty list both for a clean configuration and for settings it could not read, so the Pass wording says no conflicting setting was found rather than claiming the configuration was verified.

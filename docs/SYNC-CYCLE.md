...
| Nicotine+ | `POST /v1/porttest` via the bridge plugin | Active probe via the Soulseek project's check service - but only on versions that have one. Nicotine+ 3.3.10 and earlier expose no scriptable port checker - the `check-port-status` API was added after 3.3.10 (their built-in check is a browser redirect the plugin cannot read) - so the plugin reports the capability as unavailable and the result is undetermined. Where it is available, the plugin caps how long it holds the request below qbPortWeaver's HTTP timeout, so a slow check returns `pending` and is treated as undetermined rather than closed |
